...
export const useStockSentiment = (refreshInterval: number = 300000) => { // 5 minutes default
  ...
  const fetchStockSentiment = async () => {
    ...
    try {
      const stockPromises = TOP_10_STOCKS.map(async (symbol) => {
        const response = await fetch(`/api/proxy/finnhub/quote?symbol=${symbol}`);
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch stock sentiment');
